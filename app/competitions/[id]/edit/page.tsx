...
export default function EditCompetitionPage() {
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch(`/api/competitions/${id}`, { cache: 'no-store' })
      if (!res.ok) { setError('Compétition introuvable.'); setLoading(false); return }
      const data = await res.json()
      setLocation(data.location || '')
      setName(data.name || '')
      setDate(data.date || '')
      setNotes(data.notes || '')
      setSelectedEvents(new Set(data.entries.map((e: { stroke: string; distance: number }) => `${e.stroke}-${e.distance}`)))
      setLoading(false)
    }
    load()
  }, [id])
  ...
  async function handleSave() {
    if (!location.trim() && !name.trim()) { setError(tr(language, 'competition.new.location')); return }
    setSaving(true)
    try {
  ...
  if (loading) return <div style={{ paddingTop: 80, color: 'var(--text-muted)' }}>{tr(language, 'common.loading')}</div>

  return (
    <div style={{ maxWidth: 720, margin: '0 auto', animation: 'fadeIn 0.4s ease-out' }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 38, fontWeight: 800, letterSpacing: '-0.04em' }}>
      ...
      <div className="card" style={{ padding: 32, display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div>
          ...
          <input value={location} onChange={(e) => setLocation(e.target.value)}
            list="swim-locations" style={inputStyle} placeholder="Duna Aréna, BVSC Uszoda…" />
        ...
        <div>
          ...
          <input value={name} onChange={(e) => setName(e.target.value)}
            style={inputStyle} placeholder={location || 'Nom de la compétition…'} />
        </div>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={handleSave} disabled={saving} className="btn-primary" style={{ flex: 1, height: 56, fontSize: 16 }}>
            {saving ? tr(language, 'common.loading') : tr(language, 'common.save')}
          </button>
          <button onClick={() => router.push(`/competitions/${id}`)} className="btn-ghost" style={{ height: 56, minWidth: 120 }}>
        ...
        <div style={{ borderTop: '1px solid var(--card-border)', paddingTop: 20 }}>
          <button onClick={handleDelete} style={{ border: '1px solid #fecaca', background: '#fee2e2', color: '#991b1b', borderRadius: 999, padding: '10px 18px', fontWeight: 800, cursor: 'pointer', fontSize: 14 }}>
            {tr(language, 'admin.swimmer.edit.delete').replace('nageur', 'compétition')}
          </button>
        </div>
      </div>
    </div>
